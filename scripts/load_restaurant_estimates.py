#!/usr/bin/env python3
"""
Load restaurant ETA estimates from a CSV/TSV file into MongoDB via the eta-service HTTP API.

Each CSV row is processed independently:
  1. POST /api/v1/eta/restaurant  (insert doc with this row's meal filled, others zero)
  2. On HTTP 409 (doc exists) → PATCH /api/v1/eta/restaurant/:restaurantId for that meal
...
Expected CSV columns (tab or comma separated):
  sublocalityId, cityId, zoneId, dayType, mealType, restaurantId,
  rat_seconds, rat_sample_count, kpt_seconds, kpt_sample_count,
  pickup_seconds, pickup_sample_count, delay_dispatch_seconds,
  delay_dispatch_sample_count
...
def meal_from_row(row: dict[str, str]) -> dict[str, Any]:
    return {
        "rat": {
            "seconds": parse_float(row["rat_seconds"]),
            "sampleCount": parse_int(row["rat_sample_count"]),
        },
        "kpt": {
            "seconds": parse_float(row["kpt_seconds"]),
            "sampleCount": parse_int(row["kpt_sample_count"]),
        },
        "pickup": {
            "seconds": parse_float(row["pickup_seconds"]),
            "sampleCount": parse_int(row["pickup_sample_count"]),
        },
        "delayDispatch": {
            "seconds": parse_float(row["delay_dispatch_seconds"]),
            "sampleCount": parse_int(row["delay_dispatch_sample_count"]),
        },
    }
...
def build_insert_payload(
    ...
    return {
        "restaurantId": row["restaurantId"].strip(),
        "dayType": day_types,
        "cityId": city_id,
        "zoneId": zone_id,
...
def build_patch_payload(row: dict[str, str], meal_api: str, day_types: list[str]) -> dict[str, Any]:
    payload: dict[str, Any] = {
        ...
        "mealType": meal_api,
        "ratSeconds": parse_float(row["rat_seconds"]),
        "ratSampleCount": parse_int(row["rat_sample_count"]),
        "kptSeconds": parse_float(row["kpt_seconds"]),
        "kptSampleCount": parse_int(row["kpt_sample_count"]),
        "pickupSeconds": parse_float(row["pickup_seconds"]),
        "pickupSampleCount": parse_int(row["pickup_sample_count"]),
        "delayDispatchSeconds": parse_float(row["delay_dispatch_seconds"]),
        "delayDispatchSampleCount": parse_int(row["delay_dispatch_sample_count"]),
    }

    # Replace dayType when CSV specifies multiple days — needed when INSERT
    # returned 409 because the doc already existed with a subset of days.
    if len(day_types) > 1:
...
def process_row(
    ...
    restaurant_id = row["restaurantId"].strip()
    if not restaurant_id:
        return "failed", "missing restaurantId"
    ...
    insert_url = f"{base_url.rstrip('/')}/api/v1/eta/restaurant"
    ...
    if status == 409:
        patch_url = f"{base_url.rstrip('/')}/api/v1/eta/restaurant/{restaurant_id}"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Load restaurant ETA estimates from CSV via eta-service HTTP API.",
    )
    parser.add_argument("--file", required=True, help="Path to data.csv (tab or comma separated)")
    ...
    for index, row in enumerate(rows, start=2):
        ...
        restaurant_id = row.get("restaurantId", "").strip()
        meal_type = row.get("mealType", "").strip()
        if status == "failed":
            print(f"row {index} {restaurant_id} {meal_type}: FAILED — {detail}", file=sys.stderr)
            if args.fail_fast:
                break
        else:
            print(f"row {index} {restaurant_id} {meal_type}: {status}")
